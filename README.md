...
- **Rootkit check** runs on each daemon scan and from `aftersec forensics rootkit`. On macOS it compares the kernel process table with `ps` and reports a loaded kernel extension that is not an Apple bundle. On Linux it compares `/proc` directory listing with direct `stat`, `/proc/modules` with `/sys/module`, and TCP socket inodes with process file descriptors. A system-wide preload is reported. A per-process preload is reported when the library is outside the operating-system library directories. A view that cannot be read is unavailable, not a clean result. The check does not time syscalls or read kernel memory.
