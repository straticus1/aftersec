...
// ProgressEvent represents a progress update event for GUI integration
type ProgressEvent struct {
	Type      string                 `json:"type"`      // scan_start, file_scanning, file_scanned, threat_detected, scan_complete, scan_error
	Timestamp time.Time              `json:"timestamp"`
	Data      map[string]interface{} `json:"data,omitempty"`
}

// ProgressReporter handles progress reporting for GUI integration
// Emits JSON events to stderr for real-time monitoring
...
// GUI applications can parse these events for real-time updates
...
func (pr *ProgressReporter) FileScanned(result *ScanResult) {
	pr.scannedFiles++
...
func (pr *ProgressReporter) ThreatDetected(result *ScanResult) {
	if !result.Infected || len(result.Threats) == 0 {
		return
	}
...
// UpdateProgress updates internal counters and emits progress
func (pr *ProgressReporter) UpdateProgress(scanned, total, threats int) {
	pr.scannedFiles = scanned
	pr.totalFiles = total
	pr.threatsFound = threats

	pr.emit("progress_update", map[string]interface{}{
		"progress": pr.getProgress(),
	})
}

// getProgress calculates current progress metrics
func (pr *ProgressReporter) getProgress() map[string]interface{} {
	...
	if pr.scannedFiles > 0 && elapsed > 0 {
