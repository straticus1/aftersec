...
- **Endpoint install**: `deploy/bootstrap.py` is the only file a new machine receives. The server substitutes the publisher Ed25519 public key at `GET /api/v1/bootstrap/install.py` and sets `X-Aftersec-Bootstrap-SHA256`. The script verifies the manifest with its own Ed25519 check, then downloads `GET /api/v1/bootstrap/artifacts/<sha256>` from that same HTTPS origin. The publisher is off unless `AFTERSEC_BOOTSTRAP_PUBLIC_KEY`, `AFTERSEC_BOOTSTRAP_MANIFEST`, and `AFTERSEC_BOOTSTRAP_DIR` are set. The signing private key stays off the server. `aftersec enroll <code>` still requires a hardware attestation quote. Windows `amd64` can install `aftersec-windows.exe` the same way; `report` posts read-only posture to `POST /api/v1/inventory/windows` and the row stays `inventory` with no certificate, no refresh token, and no remote actions.
